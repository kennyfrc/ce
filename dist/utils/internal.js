...
export var updateTable = function () {
    var obj = this;
    // Check for spare
 ...
 */
var parseNumber = function (value, columnNumber) {
    var obj = this;
    // Decimal point
 ...
 * Parse formulas
 */
export var executeFormula = function (expression, x, y) {
    var obj = this;
    var formulaResults = [];
    ...
    var execute = function (expression, x, y) {
        ...
        var parentId = getColumnNameFromId([x, y]);
        // Code protection
        ...
        var tokensUpdate = function (tokens) {
            for (var index = 0; index < tokens.length; index++) {
                ...
                var e1 = getIdFromColumnName(token[0], true);
                var e2 = getIdFromColumnName(token[1], true);
        ...
        if (tokens && tokens.indexOf(parentId) > -1) {
            ...
            if (tokens) {
                for (var i = 0; i < tokens.length; i++) {
                    ...
                    if (eval('typeof(' + tokens[i] + ') == "undefined"')) {
                        // Coords
                        var position = getIdFromColumnName(tokens[i], 1);
                        // Get value
                        var value = void 0;
                        ...
                        if (('' + value).trim() == '') {
                            ...
                            if (value == Number(value) && obj.parent.config.autoCasting != false) {
                                ...
                                var number = parseNumber.call(obj, value, position[0]);
            ...
            var ret = dispatch.call(obj, 'onbeforeformula', obj, expression, x, y);
            if (ret === false) {
                return expression;
            }
            else if (ret) {
...
export var parseValue = function (i, j, value, cell) {
    var obj = this;
    if (('' + value).substr(0, 1) == '=' && obj.parent.config.parseFormulas != false) {
        value = executeFormula.call(obj, value, i, j);
    }
    // Column options
    var options = obj.options.columns && obj.options.columns[i];
 ...
 */
var getDropDownValue = function (column, key) {
    var obj = this;
    var value = [];
...
export var createCell = function (i, j, value) {
    var obj = this;
    ...
    if (obj.options.columns && obj.options.columns[i] && typeof obj.options.columns[i].type === 'object') {
        ...
        if (obj.options.columns && obj.options.columns[i] && obj.options.columns[i].type == 'hidden') {
            ...
            var formatted = null;
            if (!validDate(value)) {
                var tmp = jSuites.calendar.extractDateFromString(value, (obj.options.columns[i].options && obj.options.columns[i].options.format) || 'YYYY-MM-DD');
                if (tmp) {
                    formatted = tmp;
                }
            }
    ...
    // Text align
    var colAlign = (obj.options.columns && obj.options.columns[i] && obj.options.columns[i].align) || obj.options.defaultColAlign || 'center';
 ...
 * @return void
 */
export var updateCell = function (x, y, value, force) {
    var obj = this;
    var record;
    ...
    if (obj.records[y][x].element.classList.contains('readonly') == true && !force) {
        ...
        // On change
        var val = dispatch.call(obj, 'onbeforechange', obj, obj.records[y][x].element, x, y, value);
        ...
        if (obj.options.columns && obj.options.columns[x] && typeof obj.options.columns[x].type === 'object' && typeof obj.options.columns[x].type.updateCell === 'function') {
            var result = obj.options.columns[x].type.updateCell(obj.records[y][x].element, value, parseInt(x), parseInt(y), obj, obj.options.columns[x]);
            if (result !== undefined) {
                value = result;
            }
        }
        ...
        var editor = obj.options.columns && obj.options.columns[x] && typeof obj.options.columns[x].type === 'object' ? obj.options.columns[x].type : null;
        if (editor) {
            ...
            if (obj.options.columns && obj.options.columns[x] && (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio')) {
                ...
                var formatted = null;
                if (!validDate(value)) {
                    var tmp = jSuites.calendar.extractDateFromString(value, (obj.options.columns[x].options && obj.options.columns[x].options.format) || 'YYYY-MM-DD');
                    if (tmp) {
                        formatted = tmp;
                    }
                }
        ...
        if (obj.options.columns && obj.options.columns[x] && typeof obj.options.columns[x].render === 'function') {
            obj.options.columns[x].render(obj.records[y] && obj.records[y][x] ? obj.records[y][x].element : null, value, parseInt(x), parseInt(y), obj, obj.options.columns[x]);
        }
        // On change
 ...
 */
export var isFormula = function (value) {
    var v = ('' + value)[0];
...
export var updateFormulaChain = function (x, y, records) {
    var obj = this;
    var cellId = getColumnNameFromId([x, y]);
    if (obj.formula[cellId] && obj.formula[cellId].length > 0) {
        if (chainLoopProtection[cellId]) {
            ...
            for (var i = 0; i < obj.formula[cellId].length; i++) {
                var cell = getIdFromColumnName(obj.formula[cellId][i], true);
                // Update cell
 ...
 * Update formula
 */
export var updateFormula = function (formula, referencesToUpdate) {
    var testLetter = /[A-Z]/;
    var testNumber = /[0-9]/;
    var newFormula = '';
    var letter = null;
 ...
 * Update formulas
 */
var updateFormulas = function (referencesToUpdate) {
    var obj = this;
    // Update formulas
    for (var j = 0; j < obj.options.data.length; j++) {
        for (var i = 0; i < obj.options.data[0].length; i++) {
            ...
            if (value.substr(0, 1) == '=') {
                // Replace tokens
                var newFormula = updateFormula(value, referencesToUpdate);
                if (newFormula != value) {
 ...
 * @return void
 */
export var updateTableReferences = function () {
    var obj = this;
    ...
    for (var j = 0; j < obj.rows.length; j++) {
        if (obj.rows[j]) {
            var y = obj.rows[j].element.getAttribute('data-y');
            if (y != j) {
                // Update coords
    ...
    var updatePosition = function (x, y, i, j) {
        ...
        // Other updates
        if (x != i || y != j) {
            var columnIdFrom = getColumnNameFromId([x, y]);
            var columnIdTo = getColumnNameFromId([i, j]);
    ...
    for (var j = 0; j < obj.records.length; j++) {
        for (var i = 0; i < obj.records[0].length; i++) {
            if (obj.records[j][i]) {
                // Current values
                var x = obj.records[j][i].element.getAttribute('data-x');
                var y = obj.records[j][i].element.getAttribute('data-y');
                // Update column
 ...
 */
export var updateScroll = function (direction) {
    var obj = this;
    ...
    var reference = obj.records[obj.selectedCell[3]][obj.selectedCell[2]].element;
    // Reference
    ...
    // Freeze columns?
    var freezed = getFreezeWidth.call(obj);
...
export var updateResult = function () {
    var obj = this;
    var total = 0;
    var index = 0;
    // Page 1
 ...
 * @return string value
 */
export var getCell = function (x, y) {
    var obj = this;
    if (typeof x === 'string') {
        // Convert in case name is excel liked ex. A10, BB92
        var cell = getIdFromColumnName(x, true);
        x = cell[0];
        y = cell[1];
    }
 ...
 * @return string value
 */
export var getCellFromCoords = function (x, y) {
    var obj = this;
    return obj.records[y][x].element;
};
/**
 ...
 * @return string value
 */
export var getLabel = function (x, y) {
    var obj = this;
    if (typeof x === 'string') {
        // Convert in case name is excel liked ex. A10, BB92
        var cell = getIdFromColumnName(x, true);
 ...
 */
export var fullscreen = function (activate) {
 ...
 */
export var createNestedHeader = function (nestedInformation) {
    var obj = this;
    ...
    for (var i = 0; i < nestedInformation.length; i++) {
        ...
        td_1.setAttribute('data-column', column.join(','));
        td_1.setAttribute('colspan', nestedInformation[i].colspan);
        td_1.setAttribute('align', nestedInformation[i].align || 'center');
        td_1.setAttribute('id', nestedInformation[i].id);
        td_1.textContent = nestedInformation[i].title;
        tr.appendChild(td_1);
    }
    return tr;
};
export var getWorksheetActive = function () {
    var spreadsheet = this.parent ? this.parent : this;
    return spreadsheet.element.tabs ? spreadsheet.element.tabs.getActive() : 0;
};
export var getWorksheetInstance = function (index) {
    var spreadsheet = this;
    var worksheetIndex = typeof index !== 'undefined' ? index : getWorksheetActive.call(spreadsheet);

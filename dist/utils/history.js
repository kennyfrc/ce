 ...
 * @return null
 */
export var setHistory = function (changes) {
    var obj = this;
 ...
 * Process row
 */
var historyProcessRow = function (type, historyRecord) {
    var obj = this;
    ...
    if (type == 1) {
        ...
        // Insert data
        var records = historyRecord.rowRecords.map(function (row) {
            return __spreadArray([], row, true);
        });
        obj.records = injectArray(obj.records, rowIndex, records);
        var data = historyRecord.rowData.map(function (row) {
            return __spreadArray([], row, true);
        });
 ...
 * Process column
 */
var historyProcessColumn = function (type, historyRecord) {
    var obj = this;
    ...
    if (type == 1) {
        ...
        for (var j = 0; j < historyRecord.data.length; j++) {
            ...
            var index_1 = 0;
            for (var i = columnIndex; i < (historyRecord.numOfColumns + columnIndex); i++) {
                obj.rows[j].element.insertBefore(historyRecord.records[j][index_1].element, obj.rows[j].element.children[i + 1]);
                index_1++;
            }
        }
        // Process footers
    ...
    for (var j = 0; j < obj.records.length; j++) {
        for (var i = columnIndex; i < obj.records[j].length; i++) {
            obj.records[j][i].x = i;
        }
    }
...
export var undo = function () {
    ...
    var ignoreEvents = obj.parent.ignoreEvents ? true : false;

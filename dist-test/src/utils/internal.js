    ...
    if (obj.formula && obj.formula[cellId] && obj.formula[cellId].length > 0) {
        if (chainLoopProtection.hasOwnProperty(cellId)) {
            ...
            for (let i = 0; i < obj.formula[cellId].length; i++) {
                ...
                // Update cell
                let cellValue = "";
                if (obj.options.data && Array.isArray(obj.options.data) && obj.options.data[cell[1]] && Array.isArray(obj.options.data[cell[1]]) && typeof cell[0] === 'number' && cell[0] < obj.options.data[cell[1]].length && obj.options.data[cell[1]][cell[0]] !== undefined) {
...
const createNestedHeader = function (nestedInformation) {
    ...
    for (let i = 0; i < nestedInformation.length; i++) {
        ...
        let numberOfColumns = typeof colspanValue === 'number'
            ? colspanValue

...
const historyProcessRow = function (type, historyRecord) {
    ...
    if (type == 1) {
        ...
        obj.rows = (0, internalHelpers_1.injectArray)(obj.rows, rowIndex, historyRecord.rowNode);
        // Insert nodes
        let index = 0;
...
const historyProcessColumn = function (type, historyRecord) {
    ...
    if (type == 1) {
        ...
        // Insert data
        obj.options.columns = (0, internalHelpers_1.injectArray)(obj.options.columns, columnIndex, historyRecord.columns);
        obj.headers = (0, internalHelpers_1.injectArray)(obj.headers, columnIndex, historyRecord.headers);
        obj.cols = (0, internalHelpers_1.injectArray)(obj.cols, columnIndex, historyRecord.cols);
        let index = 0;
        ...
        for (let j = 0; j < historyRecord.data.length; j++) {
            ...
            obj.records[j] = (0, internalHelpers_1.injectArray)(obj.records[j], columnIndex, historyRecord.records[j]);
            let index = 0;
            for (let i = columnIndex; i < historyRecord.numOfColumns + columnIndex; i++) {
        ...
        if (obj.options.footers) {
            for (let j = 0; j < obj.options.footers.length; j++) {
                obj.options.footers[j] = (0, internalHelpers_1.injectArray)(obj.options.footers[j], columnIndex, historyRecord.footers[j]);
            }
        }
    }
    ...
    if (obj.options.nestedHeaders &&
        ...
        for (let j = 0; j < obj.options.nestedHeaders.length; j++) {
            ...
            if (type == 1) {
                colspan =
                    parseInt(obj.options.nestedHeaders[j][obj.options.nestedHeaders[j].length - 1].colspan) - historyRecord.numOfColumns;
            }
            else {
                colspan =
                    parseInt(obj.options.nestedHeaders[j][obj.options.nestedHeaders[j].length - 1].colspan) + historyRecord.numOfColumns;
...
const undo = function () {
    ...
    if (obj.historyIndex >= 0) {
        ...
        if (historyRecord.action == "insertRow") {
            ...
            orderBy_1.updateOrderArrow.call(obj, historyRecord.column, historyRecord.order ? 0 : 1);
...
const redo = function () {
    ...
    if (obj.historyIndex < obj.history.length - 1) {
        ...
        if (historyRecord.action == "insertRow") {
            ...
            merges_1.setMerge.call(obj, historyRecord.column, historyRecord.colspan, historyRecord.rowspan, 1);
        }
        else if (historyRecord.action == "setStyle") {
            ...
            orderBy_1.updateOrderArrow.call(obj, historyRecord.column, historyRecord.order);
